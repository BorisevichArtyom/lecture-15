...
//        jdbcMock = Mockito.mock(JdbcTemplate.class);
        ...
        Mockito.lenient().when(jdbcMock.queryForObject(anyString(), Mockito.<BeanPropertyRowMapper<Muscle>>any(),
                anyInt())).thenReturn(muscle);
        Mockito.lenient(). when(jdbcMock.query(anyString(), Mockito.<RowMapper<Muscle>>any(),
    ...
    public void testGetMuscleById() throws DAOException {
        ...
        Assertions.assertEquals(muscle.getMuscleName(), muscleDAO.getMuscleById(2).getMuscleName());
    ...
    public void testAddMuscle() throws DAOException {
        muscleDAO = Mockito.mock(MuscleDAOImpl.class);
        doNothing().when(muscleDAO).addMuscle(any());
